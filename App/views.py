...
def transcribe_with_gemini(audio_path, language=None, task="transcribe", target_language=None):
    ...
    try:
        ...
        try:
            ...
            default_offset_ms = 3500  # 3.5 second delay to account for common synchronization issues
            ...
            return [{
                'start': 3500,  # Start after 3.5 seconds to avoid early display
                'end': 13500,  # 10 seconds default
...
def upload(request):
    ...
    if request.method == "POST":
        ...
        subtitle_offset = request.POST.get('subtitle_offset', '3.5')  # Default to 3.5 seconds
        try:
            subtitle_offset_ms = int(float(subtitle_offset) * 1000)  # Convert to milliseconds
        except ValueError:
            subtitle_offset_ms = 3500  # Default to 3.5 seconds if invalid
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=os.path.splitext(video.name)[1], delete=False) as temp_video:
                ...
                if subtitle_offset_ms != 0 and subtitle_offset_ms != 3500:  # Don't double-apply if it's the default
                    transcribed_segments = apply_time_offset(transcribed_segments, subtitle_offset_ms - 3500)
                
                # Debug output
                print(f"Got {len(transcribed_segments)} segments from transcribe_with_gemini")
                if transcribed_segments:
                    print(f"First segment: {transcribed_segments[0]}")
                else:
                    print("No segments returned from transcribe_with_gemini")
                ...
                if output_format == 'txt':
                    ...
                    with open(output_path, 'w', encoding='utf-8') as f:
                        for sub in transcribed_segments:
                    ...
                    with open(output_path, 'w', encoding='utf-8') as f:
                        ...
                        for i, sub in enumerate(transcribed_segments, start=1):
                            ...
                            # Add cue identifier for better compatibility
                            f.write(f"{i}\n")
                            f.write(f"{start} --> {end}\n")
                            f.write(f"{sub['text']}\n\n")
                        ...
                        for sub in transcribed_segments:
                            ...
                            for word in words:
                                ...
                                if doc.stringWidth(test_line, "Helvetica", 12) < width - 144:
                                    ...
                                    doc.drawString(72, y_position, line)
                                    y_position -= 15
                                    line = word
                ...
                subtitle.actions = 'transcribed video'
