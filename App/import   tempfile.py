...
def detect_language_with_gemini(audio_path):
    """
    Detect language using Gemini API with fallback for quota limits
    """
    try:
        ...
        if "429" in str(e) and "quota" in str(e).lower():
            print("Quota limit reached. Using fallback language detection...")
            
            # For Hindi to English translation, we can assume Hindi as source
            # This is based on the user's specific use case
            return ('hi', 'Hindi')
        
        return ('en', 'English')  # Default to English on error

def generate_fallback_subtitles(audio_duration, source_lang='hi', target_lang='en'):
    """
    Generate fallback subtitles when API calls fail
    This creates basic timed segments that can be manually edited later
    ...
    # Create segments of approximately 5 seconds each
    segment_duration = 5000  # 5 seconds in milliseconds
    segment_count = max(1, int(audio_duration / 5))
    
    source_lang_name = language_names.get(source_lang, 'Unknown')
    target_lang_name = language_names.get(target_lang, 'English')
    
    for i in range(segment_count):
        start_time = i * segment_duration
        end_time = min((i + 1) * segment_duration, audio_duration * 1000)
        
        # Create placeholder text
        if source_lang == target_lang:
            text = f"[Transcription placeholder {i+1}. Edit this text.]"
        else:
            text = f"[Translation from {source_lang_name} to {target_lang_name} placeholder {i+1}. Edit this text.]"
...
def transcribe_with_gemini(audio_path, language=None, task="transcribe", target_language=None):
    """
    Transcribe or translate audio using Gemini API with fallback for quota limits
    ...
    """
    try:
        # Get audio duration for fallback mechanism
        audio_duration = 0
        try:
        ...
        # Check if this is a quota limit error
        if "429" in str(e) and "quota" in str(e).lower():
            print("Quota limit reached. Using fallback subtitle generation...")
            
            # Generate fallback subtitles
            return generate_fallback_subtitles(audio_duration, language, target_language)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=os.path.splitext(video.name)[1], delete=False) as temp_video:
                ...
                subtitle.duration = audio_duration
                subtitle.save()
                ...
                if source_lang == 'auto':
                    # For Hindi to English translation, we can assume Hindi as source
                    # if the user specifically mentioned this use case
                    detected_lang_code, detected_lang_name = detect_language_with_gemini(audio_path)
                    
                    # If language detection fails due to quota limits, default to Hindi for this specific use case
                    if detected_lang_code == 'en' and not detected_lang_name:
                ...
                # If no segments were returned (complete failure), create basic placeholders
                if not transcribed_segments:
                    transcribed_segments = generate_fallback_subtitles(audio_duration, source_lang, target_lang)
                ...
                subtitle.processing_duration = processing_duration
                subtitle.save()
